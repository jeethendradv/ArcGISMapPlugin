...
function CordovaArcGISMap(execCmd) {
  var self = this;
  ...
  self.on('isSuspended_changed', function(oldValue, newValue) {
    if (newValue) {
      cordova_exec(null, null, 'CordovaArcGISMap', 'pause', []);
    } else {
      cordova_exec(null, null, 'CordovaArcGISMap', 'resume', []);
    }
  });
}

utils.extend(CordovaArcGISMap, BaseClass);

CordovaArcGISMap.prototype.traceDomTree = function(element, elemId, isMapChild) {
  //------------------------------------------
...
CordovaArcGISMap.prototype.setDomId = function(element) {
  //----------------------------------------------------------------------
...
CordovaArcGISMap.prototype.putHtmlElements = function() {
  //----------------------------------------------------------------------
  ...
  cordova_exec(function() {
    ...
    self.isChecking = false;
    self.pause();
  }, null, 'CordovaArcGISMap', 'putHtmlElements', [self.domPositions]);
};


CordovaArcGISMap.prototype.pause = function() {
  var self = this;
...
CordovaArcGISMap.prototype.resume = function() {
  var self = this;
  self.set('isSuspended', false);
};
...
CordovaArcGISMap.prototype.followMaps = function(evt) {
  var self = this;
  if (self.MAP_CNT === 0) {
    return;
  }
...
CordovaArcGISMap.prototype.onTransitionEnd = function(evt) {
  var self = this;
...
CordovaArcGISMap.prototype.detectTransitionFinish = function() {
  var self = this;
...
CordovaArcGISMap.prototype.onTransitionFinish = function() {
  var self = this;
  if (self.MAP_CNT === 0) {
    self.transforming = false;
    return;
...
CordovaArcGISMap.prototype.removeDomTree = function(node) {
  //----------------------------------------------------------------------------
...
CordovaArcGISMap.prototype.invalidate = function(opts) {
  //-------------------------------
...
CordovaArcGISMap.prototype.followMapDivPositionOnly = function(opts) {
  //----------------------------------------------------------------------------
  ...
  if (changed || opts.force) {
    cordova_exec(null, null, 'CordovaArcGISMap', 'updateMapPositionOnly', [mapRects]);
    return changed;
  }
  return false;
};

CordovaArcGISMap.prototype.invalidateN = function(cnt) {
  var self = this;
  if (self.cnt > 0) {
    return;
  }
...
CordovaArcGISMap.prototype.getMap = function(div, mapOptions) {

  //----------------------------------------------------------------------------
  // This procedure return a map instance.
  //   - usage 1
  //       com.experieco.plugin.Map.getMap(options?) returns a map instance.
  //
  //   - usage 2
  //       com.experieco.plugin.Map.getMap(mapDiv, options?) returns a map instance.
  ...
  window.plugin.arcgis.maps[mapId] = nativeCallback.bind(map);
...
CordovaArcGISMap.prototype._remove = function(mapId) {
  var self = this;
  delete window.plugin.arcgis.maps[mapId];
  var map = self.MAPS[mapId];
  if (map) {
    var div = map.getDiv();
    if (!div) {
...
function postMapInit(map, div, options) {
  ...
  if (common.isDom(div)) {
    ...
    if (div.offsetWidth < 100 || div.offsetHeight < 100) {
      console.error('[ArcGISMap] Minimum container dimention is 100x100 in pixels.', div);
      return;
    }
...
module.exports = CordovaArcGISMap;
